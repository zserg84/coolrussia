 ...
 * @var \modules\comments\models\frontend\Comment[] $models Comments models
 */
...
<?php if ($models !== null) : ?>
    <?php foreach ($models as $comment) : ?>
        <div class="media" data-comment="parent" data-comment-id="<?= $comment->id ?>">
            <?php
            $avatar = ($comment->author->image_id) ? $comment->author->image->getSrc() : Yii::$app->assetManager->publish('@modules/themes/site/assets/images/blog/avatar3.png')[1]; ?>
            <div class="pull-left">
                <img src="<?= $avatar ?>" class="avatar img-circle width-50" alt="<?= $comment->author->name ?>"/>
            </div>
