...
namespace modules\geo\components;
...
class IPGeoBase {
	...
	{
		if (!$CIDRFile) {
			$CIDRFile = Yii::getAlias('@modules/geo/data/cidr_optim.txt');
		}
		if (!$CitiesFile) {
			$CitiesFile = Yii::getAlias('@modules/geo/data/cities.txt');
		}
	...
	public static function GetCountryByIP($ip) {
		...
		$table_europe = Module::TABLE_COUNTRY_EUROPE;
		$table_world = Module::TABLE_COUNTRY_WORLD;
		/** @var Connection $db */
		$db = Yii::$app->db;
		$params = [':ip'=>[$int, PDO::PARAM_INT]];
		// ищем в Европе
		$country_id = $db->createCommand('SELECT `country_id` FROM (SELECT * FROM '.$table_europe.' WHERE `begin_ip` <= :ip ORDER BY begin_ip DESC LIMIT 1) AS t WHERE `end_ip` >= :ip', $params)->queryScalar();
		if (!$country_id) {
			// если не находим, то ищем во всём мире
			$country_id = $db->createCommand('SELECT `country_id` FROM (SELECT * FROM '.$table_world.' WHERE `begin_ip` <= :ip ORDER BY `begin_ip` DESC LIMIT 1) AS t WHERE `end_ip` >= :ip', $params)->queryScalar();
		}
		return $country_id;
	}
}
